...
function ChatContent({ chatId, currentUserId, conversation }) {
  ...
  // Fetch tin nhắn khi chatId thay đổi
  useEffect(() => {
    if (!chatId) return;

    setLoading(true);
    fetch(`https://kltn.azurewebsites.net/api/conversations/conversation/${chatId}`, {
  ...
  // Khởi tạo SignalR connection, join nhóm chat và lắng nghe message mới
  useEffect(() => {
    if (!chatId) return;
    ...
    connection.current.on("ReceiveMessage", (conversationId, senderId, message) => {
  ...
  const handleSendMessage = () => {
    ...
    fetch("https://kltn.azurewebsites.net/api/messages/send", {
      ...
      .then((data) => {
        setNewMessage("");
        // Bạn có thể đẩy message mới vào state, hoặc để SignalR nhận update realtime
          ...
          return (
            <div key={msg.id} className={`message ${isSent ? "sent" : "received"}`}>
              <div className="message-info">
                <span className="sender">{isSent ? "Bạn" : msg.senderName || "Người gửi"}</span>
